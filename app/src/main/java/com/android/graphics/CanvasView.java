...
public class CanvasView extends View {
    ...
    private void drawText(Canvas canvas) {
        ...
        for (int i = 0, len = this.text.length(); i < len; i += modNumChars) {
            ...
            y += this.fontSize;
            int textx= (int) this.textX;
            int texty= (int) this.textY;

           /* LinearLayout mLinearLayout = (LinearLayout)getRootView().findViewById(R.id.test1);
            //EditText lEditText = new EditText(getContext());
            ...
            lEditText.setWidth(40);     // change width
            lEditText.setHeight(40);   // change height
            lEditText.setX(textx);     // set absolute position of x
            lEditText.setY(texty);*/
...
//                            //Path p = new Path();
//                            path.moveTo(this.startX,this.startY);
//                            path.lineTo(140, 140 / 2.0f);
//                            path.lineTo(-2.0f, 140);
//                            path.lineTo(x, y);
//                            path.close();
                            path.moveTo(this.startX, this.startY);
                            path.lineTo(x, y);
                            float endPointx=event.getX();
                            float endPointy=event.getY();

                           // path.lineTo(startX, startY);
                            float deltaX =   endPointx-startX;
                            float deltaY =   endPointy-startY;
                            float frac = (float) 0.1;

                            float point_x_1 = startX + (float) ((1 - frac) * deltaX + frac * deltaY);
                            float point_y_1 = startY + (float) ((1 - frac) * deltaY - frac * deltaX);

                            float point_x_2 = endPointx;
                            float point_y_2 = endPointy;
    ...
    private void onDrawArrow(Canvas canvas) {
        float dx = Math.abs(controlX - startX);
        float dy = Math.abs(controlY - startY);
